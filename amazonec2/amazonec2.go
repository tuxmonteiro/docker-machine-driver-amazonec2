...
func (d *Driver) GetCreateFlags() []mcnflag.Flag {
	return []mcnflag.Flag{
		...
		mcnflag.IntFlag{
			Name:  "amazonec2-delay-to-launch",
...
func (d *Driver) SetConfigFromFlags(flags drivers.DriverOptions) error {
	...
	d.DelayToLaunch = int64(flags.Int("amazonec2-delay-to-launch"))
...
func (d *Driver) configureSecurityGroupPermissions(group *ec2.SecurityGroup) ([]*ec2.IpPermission, error) {
	...
	if !hasPortsInbound[fmt.Sprintf("%d/tcp", d.BaseDriver.SSHPort)] {
	...
	if !hasPortsInbound[fmt.Sprintf("%d/tcp", swarmPort)] && d.SwarmMaster {
		inboundPerms = append(inboundPerms, &ec2.IpPermission{
			IpProtocol: aws.String("tcp"),
	...
	if *group.GroupName == defaultSecurityGroup && hasTagKey(group.Tags, machineSecurityGroupName) {
		...
		// clusterManagerPorts
		if !hasPortsInbound[fmt.Sprintf("%d/tcp", clusterManagerPorts[0])] {
			inboundPerms = append(inboundPerms, &ec2.IpPermission{
				IpProtocol: aws.String("tcp"),
				FromPort:   aws.Int64(int64(clusterManagerPorts[0])),
				ToPort:     aws.Int64(int64(clusterManagerPorts[1])),
				UserIdGroupPairs: []*ec2.UserIdGroupPair{
					{
						GroupId: group.GroupId,
					},
